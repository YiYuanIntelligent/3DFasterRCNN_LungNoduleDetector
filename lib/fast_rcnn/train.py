...
class SolverWrapper(object):
    ...
    def __init__(self, solver_prototxt, roidb_train,roidb_val, output_dir,
        ...
        if pretrained_model is not None:
            ...
            self.epoch = int(pretrained_model[:-11].split('_')[-1])
    ...
    def train_model(self, max_iters):
        ...
        train_len,val_len = self.solver.net.layers[0].set_roidb(self.roidb_train, self.roidb_val,"TRAIN")
        ...
        for it in range(max_iters):
            ...
            # if it % cfg.TRAIN.SNAPSHOT_ITERS == 0:
            #     last_snapshot_iter = it
            #     model_paths.append(self.snapshot(it))

            # one epoch
            if (it + 1) % iter_per_epoch == 0:
                epoch += 1
